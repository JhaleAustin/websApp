...
const UpdateWithMulch = () => {
  ...
  const getAdminWithMulch = async (id) => {
    try {
    ...
    e.preventDefault();
        ...
        try {
            ...
            if (data.success) {
                ...
                setUpdateError('FAILED TO UPDATE WITH MULCHING DATA');
            }
        } catch (error) {
            setUpdateError(error.response?.data?.message || 'FAILED TO UPDATE WITH MULCHING DATA');
        }
    };

useEffect(() => {
    if (Object.keys(documentation).length === 0) {
        getAdminWithMulch(id)
    } else {
    ...
    if (error) {
        toast.error('FAILED TO UPDATE WITH MULCH DATA');
    ...
    if (isUpdated) {
      ...
      toast.success('WITH MULCH DATA IS UPDATED SUCCESSFULLY');
            ...
            <div className="np wrapper my-5 home-form4" style={{ width: '100%', paddingLeft: '10%', marginLeft: '10%' }}>
                <Fragment>
                    ...
                    <form className="row g-3 form4" encType="multipart/form-data">
                        <h1 className="headerNew">UPDATE WITH MULCHING DATA</h1>
                        <div className="row">
...
export default UpdateWithMulch